...
import { eq, like, desc, sql, between, and } from "drizzle-orm";
...
//     const total_amount = bloodDetails.reduce((sum, item) => sum + Number(item.amount || 0), 0);
...
//         is_paid: is_paid ?? false, // ✅ default false
...
//     res.status(201).json({ message: "Billing created successfully", data: result[0] });
//   } catch (err) {
//     console.error("❌ Create Billing Error:", err);
//     res.status(500).json({ message: "Failed to create billing", error: err.message });
//   }
// };
